...
export default function App() {
  ...
  // STATE BARU: Untuk membuka modal Log Transaksi
  ...
  // DATA DUMMY BARU: Log Transaksi (Pola Judi Online)
  ...
  return (
    <div className="flex h-screen bg-[#050B14] text-slate-300 font-sans overflow-hidden selection:bg-cyan-900 selection:text-cyan-50">
          ...
          <div className="bg-[#0A1326] border border-cyan-900/40 rounded-xl shadow-lg overflow-hidden">
            <div className="p-5 border-b border-cyan-900/40 bg-gradient-to-r from-red-950/20 to-transparent">
              <h3 className="text-cyan-400 text-sm font-semibold tracking-wide uppercase flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-red-500" /> Daftar
                Deteksi Anomali (Prioritas Tinggi)
              </h3>
            </div>
      ...
      {/* FULL-SCREEN MODAL OVERLAY (Level 1) */}
      {isModalOpen && (
            ...
            <div className="flex-1 p-6 grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-y-auto">
              ...
              <div className="space-y-6 flex flex-col">
                ...
                <div className="bg-[#1A0B14] border border-red-900/50 rounded-lg p-5 flex-1">
                  ...
                  {/* BUTTON BARU: Lihat Log Transaksi */}
                  <button
              ...
              <div className="space-y-6 flex flex-col">
                <div className="bg-gradient-to-br from-[#0A1326] to-[#0A1A3A] border border-blue-500/30 rounded-lg p-5 shadow-[0_0_15px_rgba(37,99,235,0.1)]">
                  ...
                  <p className="text-slate-300 text-sm leading-relaxed border-l-2 border-blue-500 pl-4 py-1 italic">
                    "Pola ketiadaan variasi harga dan lonjakan ekstrem di luar
                    jam operasional wajar mengindikasikan kuat bahwa QRIS F&B
                    ini digunakan sebagai loket deposit Judi Online. Tidak
                    ditemukan bukti pembelian barang riil."
                  </p>
                </div>
                ...
                <div className="bg-[#050B14] border border-cyan-900/50 rounded-lg p-5 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mb-6">
                      Pilih tindakan berdasarkan SOP Penanganan Anomali QRIS
                      (Pasal 12.B).
                    </p>
                  </div>
      ...
      {/* MODAL LAPIS 2: TABEL LOG TRANSAKSI (POLA JUDI ONLINE) */}
      {isTxModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 lg:p-12">
          {/* Backdrop yang lebih gelap agar fokus */}
          <div
            className="absolute inset-0 bg-slate-950/90 backdrop-blur-md"
            onClick={() => setIsTxModalOpen(false)}
          ></div>

          <div className="relative w-full max-w-4xl bg-[#0A1326] border border-blue-500/50 rounded-xl shadow-[0_0_50px_rgba(37,99,235,0.2)] overflow-hidden flex flex-col max-h-[85vh]">
            {/* Header Modal Log */}
            <div className="bg-[#0F1C38] px-6 py-4 border-b border-blue-900/50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-500/20 rounded-lg border border-blue-500/50 text-blue-400">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-400 font-mono">
                    Filter: 24 Jam Terakhir | Urut: Terbaru
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-auto p-6 bg-[#050B14]">
              ...
              <div className="mb-4 bg-red-950/40 border-l-4 border-red-500 p-4 rounded-r-lg">
                <h4 className="text-red-400 text-sm font-bold flex items-center gap-2 mb-1">
                  <AlertTriangle className="w-4 h-4" /> Deteksi Pola Mesin:
                </h4>
                <p className="text-xs text-red-200/80 leading-relaxed">
                  Ditemukan serangkaian transaksi di luar jam operasional normal
                  (03:00 - 04:00 WIB) dengan nominal berulang dan bulat (Rp
                  50.000). Jarak antar transaksi tidak wajar (&lt; 5 detik),
                  mengindikasikan penggunaan *bot* atau deposit massal Judi
                  Online.
                </p>
              </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#0F1C38] text-cyan-500 text-xs tracking-wider uppercase border-b border-cyan-900/50">
                    ...
                    <th className="px-4 py-3 font-medium rounded-tr-lg text-center">
                      Indikasi
                    </th>
                  </tr>
                </thead>
                <tbody className="text-sm font-mono">
                  {transactionLog.map((tx, index) => (
                    <tr
                      key={index}
                      className={`border-b border-cyan-900/20 hover:bg-[#111F3D] transition-colors ${
                        tx.isAnomaly ? "bg-red-500/5" : "bg-transparent"
                      }`}
                    >
                      ...
                      <td className="px-4 py-3 text-slate-300">{tx.sumber}</td>
                      <td className="px-4 py-3 text-center">
                        {tx.isAnomaly ? (
                          <span className="inline-block px-2 py-1 bg-red-600/20 text-red-500 text-[10px] uppercase font-bold rounded border border-red-600/30">
                            Suspicious
                          </span>
                        ) : (
                          <span className="inline-block px-2 py-1 bg-slate-800 text-slate-400 text-[10px] uppercase font-bold rounded">
                            Normal
                          </span>
                        )}
                      </td>
              ...
              <div className="mt-4 text-center">
                <p className="text-xs text-slate-500 italic">
                  Menampilkan 8 dari 850 transaksi yang difilter...
                </p>
              </div>
            </div>
          </div>
